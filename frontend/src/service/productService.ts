...
export async function getAllProducts(params: IFilterParams = {}) {
  try {
    ...
    const result = await axios.get<IPaginatedProducts>(
...
export async function setOutOfStock(id: number) {
  try {
    const result = await axios.put(
      `${import.meta.env.VITE_API_URL}products/${id}/outofstock`
    );
    console.log(result);
    return result.data;
  } catch (error: unknown) {
...
export async function setInStock(id: number) {
  try {
    const result = await axios.put(
      `${import.meta.env.VITE_API_URL}products/${id}/instock?quantity=10`
    );
    console.log(result);
    return result.data;
  } catch (error: unknown) {
